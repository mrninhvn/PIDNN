...
void PIDNN::setStartWeight(float wP, float wI, float wD)
{
  ow[0] = wP;
  ow[1] = wI;
  ow[2] = wD;
...
void PIDNN::setOutputLimit(float outMin, float outMax)
{
...
float PIDNN::predict( float measured, float expected )
{
  ...
  if (v[pos] > OUT_MAX) v[pos] = OUT_MAX;
  if (v[pos] < OUT_MIN) v[pos] = OUT_MIN;
